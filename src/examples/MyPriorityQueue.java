...
public class MyPriorityQueue<K extends Comparable<? super K>, E> implements
	...
	class PQLocator<K1 extends Comparable<? super K1>,E1> implements Locator<K, E> {

		K key;
		E elem;
		Object thisOwner = MyPriorityQueue.this;
	 ...
	 */
	private void expand() {
		PQLocator [] oldLocs = locs;
		locs = new PQLocator[locs.length*2];
		for (int i=0;i<oldLocs.length;i++) locs[i]=oldLocs[i];		
	}
...
/**
	...
	public void remove(Locator<K, E> loc) {
		...
		if (l.thisOwner != this) throw new RuntimeException("invalid locator");
		int pos = l.pos;
		swap(pos,--size);
		l.thisOwner = null;
		upHeap(pos);
	...
	public void replaceKey(Locator<K, E> loc, K newKey) {
		PQLocator<K,E> l = (PQLocator<K,E>) loc;
		if (l.thisOwner != this) throw new RuntimeException("invalid locator"+loc.element());
